...
original_image_file = 'inputimg/content_img.jpg'
style_image_file = 'inputimg/style_img.jpg'
...
def vgg_network(network_weights, init_image):
    ...
    for i, layer in enumerate(vgg_layers):
        if layer[0] == 'c':
            ...
            bias = bias.reshape(-1)
            conv_layer = tf.nn.conv2d(image, tf.constant(weights), (1, 1, 1, 1), 'SAME',name=layer)
            image = tf.nn.bias_add(conv_layer, bias)
        elif layer[0] == 'r':
            image = tf.nn.relu(image,name=layer)
        else:
            image = tf.nn.max_pool(image, (1, 2, 2, 1), (1, 2, 2, 1), 'SAME',name=layer)
        network[layer] = image
    return(network)
...
shape = (1,) + original_image.shape# [batch_size=1,height,weight,n_channels]
